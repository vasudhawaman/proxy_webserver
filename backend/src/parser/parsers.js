...
export function generateHtmlWarning(result) {
  ...
  return `<div style="background:#ffdddd;border:1px solid #ff8888;padding:1em;margin-bottom:1em;">
    <strong>Warning:</strong> Detected <b>${
      result.total
    }</b> potentially malicious HTML keyword(s) in this response.<br>
    ${result.found
      .map((f) => `<span>${f.name}: ${f.count}</span>`)
      .join('<br>')}
  </div>`;
}
...
export function generateJsWarning(result) {
  if (result.total === 0) return '';
  return (
    `/* WARNING: Detected ${result.total} potentially malicious JavaScript keyword(s):\n` +
